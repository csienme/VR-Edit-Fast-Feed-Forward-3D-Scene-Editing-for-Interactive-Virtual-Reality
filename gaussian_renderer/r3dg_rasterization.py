...
try:
    ...
    parent_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "r3dg-rasterization")
    
    # 💥 獲取 Conda 環境的 Include 路徑
    conda_include = os.path.join(os.environ.get('CONDA_PREFIX', ''), 'include')
    
    _C = load(
        name='r3dg_rasterization',
        extra_cuda_cflags=[
            "-I" + conda_include,  # 💥 強制第一優先：只吸入 Conda 的乾淨標頭檔！
