...
#
# 保留原本用法：
#   bash run_abl_360.sh kitchen ../data/Other-360/kitchen/images ../data/Other-360/kitchen/object_masks
#
# Stage 控制：
...
CONFIG_FILE="${CONFIG:-configs/abl_baseline_360.yaml}"
...
BASE_OUT="eval_results_abl_baseline/${DATASET_NAME}/${SCENE}"
...
if should_run 1; then
    ...
    python eval/abl_baseline_iggt.py \
        --config "${CONFIG_FILE}" \
        --data_path "${RGB_DIR}" \
        --mask_path "${MASK_DIR}" \
        --exp_name "${SCENE}" \
