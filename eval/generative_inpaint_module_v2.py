...
def run_diffusion_texture_generation(img_path, mask_2d, scaffold_depth):
    ...
    # 🟢 防線一：物理塗銷！直接把 RGB 圖上的紙箱塗成中性灰，徹底摧毀 VAE 的殘留記憶
    img_np = np.array(img_pil)
    img_np[mask_2d > 0] = [127, 127, 127]
    img_pil = Image.fromarray(img_np)
    
    # 🟢 防線二：強制將 Mask 轉為 0 與 255
    mask_2d_255 = (mask_2d > 0).astype(np.uint8) * 255
    ...
    # 2. 將絕對深度轉為 ControlNet 相對視差圖 (MiDaS format)
    ...
    # 3. 執行條件擴散生成
    #prompt = "A high-resolution, realistic , seamless texture, masterpiece, 8k resolution, highly detailed"
    prompt = "realistic background, seamless surface texture, consistent lighting, photorealistic"
    negative_prompt = " objects, blurry, artifacts, unnatural boundaries, distorted geometry, low quality, floating"

    print("    🎨 [紋理生成] 擴散模型正順著 3D 骨架長出紋理...")
    result_image = pipe(
        prompt=prompt,
...
def generative_multi_ref_propagation(
    ...
    for ref_idx in sorted_ref_indices:
        ...
        if ref_idx not in ref_cache:
            ...
            inpainted_rgb_ref = run_diffusion_texture_generation(ref_img_path, mask_ref, scaffold_depth)
