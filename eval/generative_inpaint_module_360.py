...
對每個 target view 的破洞 pixel:
  ...
  4. 所有 source 都不行的真死角才交給 cv2.inpaint (面積應 < 5%)

跟之前架構的關鍵差別:
  Push (refdom/multiref): ref PC scatter to target → sub-pixel splat artifact
                          需要 cv2.inpaint 補大塊 → 模糊
                          SD 結果不一致 → 3DGS 學不到
  
  Pull (this version):    target pixel 主動 sample 真實 source RGB
                          沒有 scatter
                          不依賴 SD (大多數填補是真實 RGB)
                          真死角極小

I/O 與 generative_multi_ref_propagation 完全相容,
eval_360.py 只要把 import 換成這個模組即可。
"""
import os
import cv2
import numpy as np
...
def _harvest_target_view(target_idx, views, all_cam_to_world_mat,
    ...
    if n_dead > 0:
        ...
        # 死角面積 dilate 一點點再 inpaint, 邊界更自然
        dead_u8 = dead_mask.astype(np.uint8) * 255
        kernel = np.ones((3, 3), np.uint8)
        dead_dilated = cv2.dilate(dead_u8, kernel, iterations=1)
        canvas = cv2.inpaint(canvas, dead_dilated, inpaintRadius=10,
...
def generative_multi_ref_propagation(
    ...
    eval_360.py 不需要改, 只要把 import 換成這個模組:
    ...
    if "_pullharvest_views" not in ref_cache:
        ...
        print(f"    架構: target 反投影 → pull 真實 source RGB → 死角才 cv2.inpaint")
    ...
    # 改後：放到子資料夾，跟 inpainted_*.png 完全隔開
