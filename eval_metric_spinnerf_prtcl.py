...
def get_bbox_from_mask(mask_tensor):
    ...
    # 包含極值邊界 (Exclusive slicing in Python requires +1)
...
def evaluate_metrics(args):
    print("📊 啟動 NVS 學術指標計算引擎 (BBox Crop 嚴格對齊 MVIP-NeRF/SPIn-NeRF)")
...
# 取得並排序 Render 檔名
    ...
    for img_name, mask_name in tqdm(zip(img_names, mask_names), total=len(img_names), desc="Evaluating"):
        ...
        # 計算 Crop 區域的 FID
        # 注意: torchmetrics FID 允許不同尺寸的 batch size=1 輸入，內部會自動 resize 至 299x299
    ...
    log_content = (
        ...
        "【 局部指標 (BBox Masked) - 對齊 MVIP/SPIn-NeRF 】\n"
