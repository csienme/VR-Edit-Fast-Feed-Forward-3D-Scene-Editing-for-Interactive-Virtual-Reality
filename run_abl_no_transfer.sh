#!/bin/bash
# w/o pixel transfer ablation：獨立 OUTPUT_ROOT，全跑 Step 1-6，不碰 golden_467
# 前提：eval/inpaint_module.py 已換成 ablation 版（無 pull harvest）
# 用法：CONFIG=configs/abl_no_transfer.yaml bash run_abl_no_transfer.sh
...
OUTPUT_ROOT="./eval_results_abl_notransfer"   # ← 獨立目錄，不碰 Full
...
for SCENE in "${SCENES[@]}"; do
    ...
    MERGED_DIR="./merged_data_abl/${SCENE}"

    echo ""
    echo "=== [${SCENE}] w/o pixel transfer | OUT=${BASE_OUT} ==="
    mkdir -p "${BASE_OUT}"

    echo "[${SCENE}] Step 1: VGGT inpainting (ablation: no transfer) + COLMAP..."
